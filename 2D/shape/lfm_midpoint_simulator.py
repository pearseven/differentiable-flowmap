...
class RW_Cache:
    def __init__(self):
        ...
        cache_s = np.zeros((self.n_steps, res_x, res_y, 1), dtype=np.float32)
        cache_adj_s = np.zeros((self.n_steps, res_x, res_y, 1), dtype=np.float32)
        cache_target_s = np.zeros((self.n_steps, res_x, res_y, 1), dtype=np.float32)
...
class LFM_Simulator:
    def __init__(self, res_x, res_y, dx,dt, reinit_every, log_dir, log_target_dir = None):
        ...
        if(add_passive_scalar):
            ...
            self.passive = ti.field(float, shape=(res_x, res_y))
            self.final_passive = ti.field(float, shape=(res_x, res_y))
            self.init_passive = ti.field(float, shape=(self.res_x, self.res_y))
            self.err_passive = ti.field(float, shape=(self.res_x, self.res_y))
            self.tmp_passive = ti.field(float, shape=(self.res_x, self.res_y))
    ...
    def step_flow_map(self,j,curr_dt,write_flow_map, write_passive, 
        ...
        if(use_short_BFECC):
            ...
            if(add_passive_scalar):
                advect_scalar_grid(self.u_x, self.u_y, self.passive, self.final_passive,  self.dx, curr_dt, self.X)
                copy_to(self.final_passive,self.passive)
