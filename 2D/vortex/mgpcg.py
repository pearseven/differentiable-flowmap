...
.. note::
    ...
    def num_fluid_neighbors(self, x, I):
        ...
        num = 2.0 * self.dim
        for i in ti.static(range(self.dim)):
            if I[i] <= 0 and (self.boundary_types[i,0] == 2 or self.boundary_types[i,0] == 1):
                num -= 1.0
            if I[i] >= dims[i] - 1 and (self.boundary_types[i,1] == 2 or self.boundary_types[i,1] == 1):
                num -= 1.0
        return num
    ...
    def solve(self,
        ...
        while max_iters == -1 or it < max_iters:
            ...
            if verbose and poisson_output_log:
...
class MGPCG_2(MGPCG):
    ...
    def apply_bc(self, u_horizontal: ti.template(), u_vertical: ti.template()):
        ...
        for i, j in u_horizontal:
            ...
            if i == 0 and self.boundary_types[0, 0] == 1:
                u_horizontal[i, j] = 0
            if i == u_dim - 1 and self.boundary_types[0, 1] == 1:
                u_horizontal[i, j] = 0
            if j == 0 and self.boundary_types[1, 0] == 1:
                u_horizontal[i, j] = 0
            if j == v_dim - 1 and self.boundary_types[1, 1] == 1:
                u_horizontal[i, j] = 0
