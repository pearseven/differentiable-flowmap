...
@ti.kernel
def calculate_viscous_force_adj(
    adj_u_x:ti.template(),
    adj_u_y:ti.template(),
    vis_x:ti.template(),
    vis_y:ti.template(),
    viscosity:float,
    adj_viscous:ti.template()
):
    for i,j in adj_u_x:
        if(viscosity!=0):
            adj_viscous[None]+=adj_u_x[i,j]*vis_x[i,j]/viscosity

    for i,j in adj_u_y:
        if(viscosity!=0):
            adj_viscous[None]+=adj_u_y[i,j]*vis_y[i,j]/viscosity
