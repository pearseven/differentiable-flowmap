...
from taichi_utils import *
import numpy as np
from hyperparameters import poisson_output_log
...
.. note::
    ...
    ``pointer`` SNode.
    '''
    def __init__(self, boundary_types, boundary_mask, N, dim=2, base_level=4, real=float):
        '''
        ...
        self.bm = [boundary_mask] + [ti.field(dtype=ti.f32)
...
class MGPCG_2_solid(MGPCG):
    ...
    def __init__(self, boundary_types, boundary_mask, boundary_vel, N, base_level=3, real=float):
        super().__init__(boundary_types, boundary_mask, N, dim=2, base_level=base_level, real=real)
    ...
    def solve_pressure_MGPCG(self, verbose):
        ...
        self.solve(max_iters=400, verbose=verbose, tol = 1.e-14)
    ...
    def Poisson(self, u_horizontal, u_vertical, verbose = False):
        ...
        self.apply_bc(u_horizontal, u_vertical)
        #add_fields(tem_p,self.p,tem_p,1.0)
        #scale_field(tem_p,dx,tem_p)
